...
// competência: obrigações, tarefas, financeiro, lançamentos a identificar
// (quando houver) e dados gerenciais do Simples Nacional (quando
// enviados). Acessada via ?painel=<clienteId>&competencia=MM/YYYY (ver
// main.jsx). Mesmo padrão visual de RelatorioCompartilhadoPage.jsx /
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', padding: '32px 16px' }}>
      <div style={{ maxWidth: 720, margin: '0 auto', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ padding: '18px 24px', background: 'var(--navy)' }}>
          ...
          <div style={{ fontSize: 18, color: '#fff', fontWeight: 700, marginTop: 4 }}>{carregando ? '...' : clienteNome}</div>
        ...
        <div style={{ padding: 24 }}>
          ...
          {!carregando && !erro && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
              ...
              {gerenciais && (
                <div>
                  <SecaoTitulo>Informações gerenciais — Simples Nacional</SecaoTitulo>
              ...
              {lancamentos.length > 0 && (
                <div>
                  <SecaoTitulo>Lançamentos a identificar</SecaoTitulo>
...
function ResumoCard({ titulo, pct, linha1, alerta }) {
  return (
    <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px' }}>
      <div style={{ fontSize: 11, color: 'var(--text3)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.03em' }}>{titulo}</div>
